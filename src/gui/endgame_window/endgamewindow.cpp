...
void EndGameWindow::on_ok_button_clicked() {
    close();
    save_statistics(erroneous_flips, difficulty_level);
}
...
void EndGameWindow::save_statistics(int err_flips, std::string difficulty) {

    score = (1.0/(((minutes*60)+seconds)*err_flips))*10000;

    std::fstream stat_file("player_statistics.txt", std::fstream::in | std::fstream::out | std::fstream::app);

    stat_file << difficulty << "," << game_start_date_time << "," << minutes << " mins "
              << seconds << " secs" << "," << err_flips << "," << score << "\n";
